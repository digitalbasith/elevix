      ...
      <div className="nav-shell">
        ...
        <nav className="desktop-nav" aria-label="Main navigation">
          <DropdownMenu modal={false}>
            ...
            <DropdownMenuContent align="start" sideOffset={14} className="nav-dropdown service-megamenu">
              ...
              {services.map((s) => <DropdownMenuItem key={s.slug} asChild><Link prefetch href={'/' + s.slug}><ServiceIcon slug={s.slug} /><span>{s.title}</span><ArrowUpRight size={15} /></Link></DropdownMenuItem>)}
        ...
        <div className="mobile-nav">
          <DropdownMenu modal={false} open={mobileOpen} onOpenChange={setMobileOpen}>
            <DropdownMenuTrigger className="menu-button" aria-label="Open navigation"><Menu size={24} /></DropdownMenuTrigger>
            <DropdownMenuContent align="end" side="bottom" sideOffset={10} collisionPadding={12} className="nav-dropdown mobile-menu-panel">
              {[
                ['Home', '/'],
              ...
              ].map(([name, url]) => <DropdownMenuItem key={url} asChild><Link href={url}>{name}<ArrowUpRight size={15} /></Link></DropdownMenuItem>)}
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
