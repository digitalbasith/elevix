...
export default async function Service({params}:{params:Promise<{slug:string}>}){const {slug}=await params;if(legacyServiceRoutes[slug])permanentRedirect('/'+legacyServiceRoutes[slug]);const service=serviceDetails.find(s=>s.slug===slug);if(!service)notFound();return <UniqueServicePage service={service}/>}
